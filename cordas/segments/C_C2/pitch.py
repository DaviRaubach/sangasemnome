...
with open('../C_A2/last_pitches.json', 'r') as openfile:
    C_A2_last_pitches = json.load(openfile)
    C_A2_last_pitches = {key: [abjad.NamedPitch(value)] for key, value in C_A2_last_pitches.items()} 
...
def after_the_wind_pitches(mat: muda.Material):
    ...
    pitches = group_pitches_by_direction(wind_x)[4:]

    def illustrate(pitches):
        staves = []
        staves.append(muda.pitches_in_staff(original_pitches))
    ...
    tail_pitches = {
        # Tail C_A2
        # "Vl1_Voice2": abjad.NamedPitch("d''''"),
        # "Vl2_Voice1": abjad.NamedPitch("g'''"),
        # "Vl2_Voice2": abjad.NamedPitch("aqf'''"),
        "Va_Voice_1": abjad.NamedPitch("d'''"),
        "Va_Voice_2": abjad.NamedPitch("cs'''"),
        "Vc_Voice_2": abjad.NamedPitch("fs'"),
        # "Cb_Voice_1": abjad.NumberedPitch(-11),
        "Cb_Voice_2": abjad.NamedPitch('d,'),
    }
    ...
    for name, _pitches in zip(all_voices, instruments_pitches):
        if name == mat.name and _pitches:
            mat.write_pitches_by_duration(
                ...
                [
                    [muda.AnnotatedDuration((11, 4), annotation="a")],
                    [muda.AnnotatedDuration((6, 4), annotation="b")],
                    [muda.AnnotatedDuration((6, 4), annotation="c")],
                    [muda.AnnotatedDuration((6, 4), annotation="d")],
                ]
            )
            # if name in tail_pitches.keys():
            #     lt = mat.plogical_ties()[0]
            #     for note in lt:
            #         note.written_pitch = tail_pitches[name]
            

    l = mat.leaf(0)
    if isinstance(l, abjad.Note) and mat.name != all_voices[-2]:  
        l.written_pitch = C_A2_last_pitches[mat.name][0]
        
    
    if mat.name == all_voices[-1]:
        mat.leaf(-1).written_pitch = abjad.NamedPitch(r"fs'")
