...
def all_rhythm(mat: muda.Material, annotated_durations, time_signatures):
    ...
    makers = [
        ...
        lambda _: muda.rmaker(rmakers.talea(_, taleas[1], 8, extra_counts=[2])),
        lambda _: muda.rmaker(rmakers.talea(_, taleas[2], 8, extra_counts=[3])),
        lambda _: muda.rmaker(rmakers.talea(_, taleas[3], 8, extra_counts=[0])),
    ...
    # if mat.name in all_voices[1] + all_voices[4]:
        # abjad.mutate.replace(mat.leaf(-1), abjad.Rest(mat.leaf(-1).written_duration))
    print("Oi")
