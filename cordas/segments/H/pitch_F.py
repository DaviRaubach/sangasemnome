...
def vnI(mat: muda.Material):
    ...
    if mat.name == all_voices[0]:
        pitches = [
            muda.ring_modulation(chord, keep_originals=False, sort=True)[3::2][
                0:n_notes
            ]
        ...
        pitches = [
            muda.ring_modulation(chord, keep_originals=False, sort=True)[4::2][
                0:n_notes
            ][-1:]
            for chord in chords.values()
        ]
...
def cb(mat: muda.Material):
    ...
    if mat.name == all_voices[-2]:
        # cb_mod = muda.transpose_outside_pitches(pitches, abjad.PitchRange("[D0, D3]"))
        # print(mat.name)
        # print(cb_mod)
        mat.write_pitches([abjad.NamedPitch(r"fqs,")])
