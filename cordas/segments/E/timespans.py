...
def illustrate(timespans):
    ...
    total_separated_mat_ts = abjad.TimespanList(
        ...
        timespans[2] + timespans[3] + timespans[4]
    )
...
def make_meters(
    total_timespans: list[abjad.Timespan], permitted_meters=None
...
def make_texture_config_alternations(
    ...
    if total_duration is None:
        total_duration = abjad.Duration(sum(mat_a) + sum(mat_b), denominator)
    ...
    alternations = [
        muda.make_alternations(total_duration, _, denominator) for _ in dur_list
    ]
    return alternations


def make_B_timespans():
    def make_patterned_alternations(
        pattern,
        voice_names,
        a_total_list,
        c_total_list,
        ...
        last_c_duration = []
        for (j, pattern), a, a_sound, b, c, c_sound in zip(
            enumerate(patterns),
            a_total_list,
            a_sound_list,
            b_list,
            ...
            r = [z[0]/2, z[0]/2, 0, 0, 0]
            s = [sum(z[2:]), 0, 0, 0, 0]
            w = [sum(y[:3]), 0, 0, y[-2], y[-1]]
            # k = [z[0]/2, z[0]/2, 0, 0, sum(z[3:])]
            # print("w", w)
            # print("k", k)
            ...
            for i in range(5):
                if i not in pattern:
                    outro += 1
                    if outro == 1:
                        alternations[voice_names[i]].append(r)
                        alternations[voice_names[i]].append(s)
                    else:
    ...
    # aplusc = [24, 24, 16, 16, 8, 16, 16,]

    # aplusc = [3, 2, 1, 1, 1, 2, 3, 1]
    aplusc = [3, 2, 1, 1]
    aplusc = [_ * 6 for _ in aplusc]
    b = [1, 2, 1, 3]
    # b = [1, 2, 1, 3, 1, 1, 1, 1]
    b = [_ * 4 for _ in b]
    a = []
    ...
    for (i, _a), _c in zip(enumerate(a), c):
        if i != 0 and i != len(a) - 1:
            new_a = _a + c_rest[-1]
            a2.append(new_a)
            c2.append(_c - int(_c / 3))
            c_rest.append(int(_c / 3))

        elif i == len(a) - 1:
            a2.append(_a)
            c2.append(_c + c_rest[-1])

        else:
            a2.append(_a)
            c_rest.append(_c - int(_c / 3))
            c2.append(int(_c / 3))
        

    pattern_2 = [1, 3, 0]
    alternations_b, durations_b = make_patterned_alternations(
        pattern_2, voices_b_names, a2, c2, b, a2, c2, 2
    )
    alternations_a.update(alternations_b)

    # for k, v in alternations_a.items():
    #     print(k)
    #     soma = 0
    #     for l in v:
    #         soma += sum(l)
    ...
    for name, alt in alternations_a.items():
        ...
        annotated_durations[name] = ts.nannotated_durations(
            subdivisions=subdivisions)

    # illustrate(total_timespans)
    annotated_durations["Time_Signatures"] = make_meters(
        total_timespans, [(2, 4), (3, 4), (4, 4)]
    )
    # print(annotated_durations["Time_Signatures"])
    # annotated_durations["Time_Signatures"] = [abjad.TimeSignature((4, 4))] * 24
...
def make_timespans(alternations, denominator):
    timespans = [
        muda.alternating_timespans(_, denominator, ["a", "b", "c"])
        for _ in alternations
    ]
